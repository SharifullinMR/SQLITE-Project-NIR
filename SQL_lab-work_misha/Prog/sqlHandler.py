...
class SqlHandler:
    ...
    def addFilter(self, comboBox: QComboBox, text: str) -> None:
        ...
        print(self.current_filter.values())
        names_filters={'Федеральный округ': ["region", "Все округа"],"Субъект": ["oblname","Все субъекты"],"Город": ["city","Все города"], "ВУЗ": ["z2", "Все ВУЗы"],"Конкурс": ["codkon", "Все конкурсы"]}
        self.filter_for_table = {key: self.current_filter.get(value[0], value[1]) for key, value in names_filters.items()}
        print(self.filter_for_table['Конкурс'])

        if not "Все конкурсы" in  self.filter_for_table['Конкурс']:
            self.query.exec(f'''SELECT k2 FROM Gr_konk where codkon= '{self.filter_for_table['Конкурс']}' ''')
            while self.query.next():
                self.filter_for_table['Конкурс']=self.query.value(0)
